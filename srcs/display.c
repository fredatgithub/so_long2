...
/*   Updated: 2022/11/20 11:37:52 by mtellami         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	display_textures(t_game *so_long, int x, int y)
{
	...
	path = get_textures_path(so_long->map->map[x][y]);
	if (!path)
		return ;
	so_long->img = mlx_xpm_file_to_image(so_long->mlx, path, &so_long->size, &so_long->size);
	mlx_put_image_to_window(so_long->mlx, so_long->win, so_long->img, y * 50, x * 50);
